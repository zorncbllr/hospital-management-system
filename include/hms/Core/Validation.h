...
std::string formatDate(std::time_t when);
std::string formatDateTime(std::time_t when);
std::string formatTime(int secondsOfDay);
std::time_t parseDate(const std::string& text);
int parseTime(const std::string& text);
